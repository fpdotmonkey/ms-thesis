...
The mechatronic tech stack theory basically held to scrutiny through the case studies.  All cases presented all layers of the stack except the EFPA robots, which had no middleware layer, though that is an interesting insight. With this model, the commissioning issues with the steering linkage Dolores could partly be diagnosed as too-tight coupling between layers.  Commissioning the Stewart platform was basically easy; by separating functions into different categories, work on any layer could be done independently.  It also raises compelling questions; can Dolores' Beckhoff industrial computer be replaced with a Raspberry Pi (only if it would support Beckhoff's middleware)?  Could the controller for the Stewart platform have been written in C or a PLC language?  What would it take to change the fieldbus protocol without the hassle seen with Dolores?  These questions are natural to ask when the tooling is presented in a structured manner as with this model.

The main issue that arose was the IO layer reaching into higher layers.  Dolores' old control system failed because it couldn't support a change in communication protocol.  The new system is over-complex because the control tools struggle to support all communication protocols.  The Stewart platform also saw issues with this; measurement signals were unexpectedly low due to an off-size resistor in the IO layer which impacted control function.  Certainly, some knowledge of the details of the IO layer's implementation must be communicated to the middleware or control layers such that correct middleware is run and sensor calibrations are accounted for.  How this could be implemented will be studied in future work.

An additional issue is that the computer layer isn't overly interesting.  Each of the case studies simply used whatever computers are available or easy to use.  In some circumstances, it could be more interesting, like to discuss which computer architecture to use for an embedded application, or whether to use an embedded Linux system like a Raspberry Pi or bare metal, or even a simple EEPROM.  Perhaps the computer layer could steal operating systems away from the middleware layer, or these case studies weren't interesting for that layer.
